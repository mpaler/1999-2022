  ...
  <div class="forum-post-info clearfix">
    ...
    <span class="forum-post-number"><?php print $permalink; ?>
      ...
      if($_SERVER['QUERY_STRING']) {
        $query_parts = $pieces = explode("=", $_SERVER['QUERY_STRING']);
        $query = array($query_parts[0] => $query_parts[1]);
      };


      $url = url($content['comment_body']['#object']->rdf_data['nid_uri'],
              array(
                'absolute' => TRUE,
                'query' => $query,
                'fragment' => 'comment-' . $content['comment_body']['#object']->cid,
              )
            );

      $link = '<a href="'.$url.'" rel="'.$url.'">#'. $content['comment_body']['#object']->cid . '</a>';
